...
public class Filters {
  ...
  void sortBorderSurfaces(int[][][] sortedBorderPoints,int[] numberBorderPoints,int[][] borderPoints,int nSurface,int[][] borderImage)
  {
    ...
    for(int i=0;i<nSurface;i++)
    {
      ...
      debug.println("initial dir = " + dir[0] + " " + dir[1]);
      actualPoint[0] = borderPoints[i][0] + dir[0];
      actualPoint[1] = borderPoints[i][1] + dir[1];
      debug.println("border Point = " + borderPoints[i][0] + " " +  borderPoints[i][1]);
      debug.println("actual Point = " + actualPoint[0] + " " + actualPoint[1]);
      ...
      while((actualPoint[0] != borderPoints[i][0]) || (actualPoint[1] != borderPoints[i][1]))
      {
        ...
        debug.println("i= "+i);
        debug.println("actual Point = " + actualPoint[0] + " " + actualPoint[1]);
        debug.println("actual Dir = " + dir[0] + " " + dir[1]);
  ...
  boolean isPartOf(int [] point, int [][][] pointList) {
    for (int i=0;i<nBorders;i++){
      for (int j=0;j<numberOfPointsPerBorder[i];j++){
        if ((pointList[i][j][0] == point[0])&&(pointList[i][j][1] == point[1])) return true;
      }
    }
    return false;
  }
  
  void initialBorderPoints(){
    println("...........dasdasdasd.............");
  ...
  //recorremos el resto de puntos hasta que encontramos uno que no pertenece a un borde recorrido y lo usamos como nuevo punto inicial.
    ...
    int [][][] borderPointList = new int[20][4800][2];
    int [][][] auxBorderPointList = new int[1][4800][2];
    int actualDepth = 0;
    ...
    for (int i=0;i<frameDimensions[0];i++) {
      for (int j=0;j<frameDimensions[1];j++) {
        ...
        int [] point = {i,j};
        int [][] borderPoints = new int[1][2];
        borderPoints[0] = point;
        if ((actualDepth!=0)&&(!isPartOf(point, borderPointList))) {
          
          initialBorderPoints[nBorders] = point;
          println("...........dasdasdasd.............");
          sortBorderSurfaces(auxBorderPointList,nPoints,borderPoints,1,reducedData);
          
          borderPointList[nBorders] = auxBorderPointList[0];
          numberOfPointsPerBorder[nBorders] = nPoints[0];
          nBorders++;
        }
      }
    }
